...
| Concurrent cap boundary | Two distinct candidates and deduplication keys observe one remaining pressure slot. | One atomic check-and-reserve selects one winner; the other records `DEFER: CAP_REACHED`, so reserved plus sent never exceeds the cap. [BRAZE-STRAT-01][BRAZE-HOLIDAY-01] |
| Cross-flow and cross-channel duplicate | Different flows or channels receive the same contact, purpose, authoritative event, and entity. | All derive the same canonical key; one reservation wins and every other candidate records `DROP: DUPLICATE`. [BRAZE-TRIG-01][HUB-WF-01] |
