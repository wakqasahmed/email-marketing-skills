...
required_lines = {
    ...
    "Pressure, suppression, and experiments": {
        "atomic cap reservation": "- In one contact-and-window-scoped atomic transaction, count sent and reserved marketing messages across channels, apply every global and segment cap, select at most one winner, and reserve its pressure slot; a concurrent loser returns `DEFER: CAP_REACHED`, and a campaign allowance may tighten but never loosen an applicable limit. [BRAZE-STRAT-01][BRAZE-HOLIDAY-01]",
        ...
        "canonical deduplication": "- Build the versioned canonical deduplication key as `v1|contact_id|purpose|event_type|event_id|entity_type|entity_id`, using the normalized authoritative event and entity IDs together and the literal `none` only when a field is not applicable; exclude flow, step, and channel so the same action collides across flows and channels, atomically reserve the key, and return `DROP: DUPLICATE` for every matching reservation or send in its validity window. [BRAZE-TRIG-01][HUB-WF-01]",
    ...
    "Behavioral acceptance traces": {
        ...
        "cap boundary trace": "| Concurrent cap boundary | Two distinct candidates and deduplication keys observe one remaining pressure slot. | One atomic check-and-reserve selects one winner; the other records `DEFER: CAP_REACHED`, so reserved plus sent never exceeds the cap. [BRAZE-STRAT-01][BRAZE-HOLIDAY-01] |",
        "cross-flow dedupe trace": "| Cross-flow and cross-channel duplicate | Different flows or channels receive the same contact, purpose, authoritative event, and entity. | All derive the same canonical key; one reservation wins and every other candidate records `DROP: DUPLICATE`. [BRAZE-TRIG-01][HUB-WF-01] |",
...
mutations = {
    ...
    "atomic cap reservation": "contact-and-window-scoped atomic transaction",
    "canonical deduplication": "exclude flow, step, and channel",
